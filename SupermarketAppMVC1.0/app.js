...
// add controller import (use the controller you asked for)
...
app.post('/login', (req, res) => {
    ...
    db.query(sql, [email, password], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            // Keep any guest cart to merge
            const guestCart = Array.isArray(req.session.cart) ? req.session.cart.slice() : [];
            req.session.user = user;

            // Load persisted cart for this user from DB and merge guest cart
            loadCartForUser(user.id, req.session, (loadErr) => {
                if (loadErr) console.error('Error loading user cart:', loadErr);

                // If guest had items, merge them into session and persist
                if (guestCart && guestCart.length > 0) {
                    // Build a map of existing session cart by product id
                    const map = new Map((req.session.cart || []).map(i => [Number(i.id), i]));
                    guestCart.forEach(git => {
                        const pid = Number(git.id);
                        const existing = map.get(pid);
                        const newQty = (existing ? existing.quantity : 0) + (git.quantity || 0);
                        // update session map and session.cart
                        if (existing) {
                            existing.quantity = newQty;
                        } else {
                            const item = { id: pid, productName: git.productName, price: git.price, quantity: newQty, image: git.image };
                            (req.session.cart = req.session.cart || []).push(item);
                            map.set(pid, item);
                        }
                        // persist the final quantity to DB
                        upsertCartItem(user.id, pid, newQty, (uErr) => { if (uErr) console.error('Error persisting merged cart item', uErr); });
                    });
                }
